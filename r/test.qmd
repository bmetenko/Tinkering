...
example_slider
Array.from(Array(Number(example_slider)))
Array.from(Array(Number(example_slider))).forEach(
  i => Array.from({length: Number(i)}, (v, w) => w)
)

Array.from(Array(example_slider)).forEach(
  function(i){
    Inputs.select(Array.from({length: i}, (v, w) => w), {label: i})
  }
)
